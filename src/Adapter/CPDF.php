...
class CPDF implements Canvas
{
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Constructing CPDF with paper: {$paper}, orientation: {$orientation}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting Dompdf instance");
        return $this->_dompdf;
    }
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting Cpdf instance");
        return $this->_pdf;
    }

    public function add_info(string $label, string $value): void
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding info: {$label} = {$value}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Opening new object");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Reopening object: {$object}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Closing object");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding object: {$object} where: {$where}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Stopping object: {$object}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Serializing object: {$id}");
        return $this->_pdf->serializeObject($id);
    }

    public function reopen_serialized_object($obj)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Reopening serialized object");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting width: {$this->_width}");
        return $this->_width;
    }

    public function get_height()
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting height: {$this->_height}");
        return $this->_height;
    }

    public function get_page_number()
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting page number: {$this->_page_number}");
        return $this->_page_number;
    }

    public function get_page_count()
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting page count: {$this->_page_count}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting page number to: {$num}");
        $this->_page_number = $num;
    }

    public function set_page_count($count)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting page count to: {$count}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting stroke color");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting fill color");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting line transparency mode: {$mode}, opacity: {$opacity}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting fill transparency mode: {$mode}, opacity: {$opacity}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting line style width: {$width}, cap: {$cap}, join: {$join}");
        $this->_pdf->setLineStyle($width, $cap, $join, $dash);
    }

    public function set_opacity(float $opacity, string $mode = "Normal"): void
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting opacity: {$opacity} mode: {$mode}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting default view: {$view}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Drawing line from ({$x1}, {$y1}) to ({$x2}, {$y2})");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Drawing arc at ({$x}, {$y}) with radii {$r1}, {$r2}");
        $this->_set_stroke_color($color);
        $this->_set_line_style($width, $cap, "", $style);
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Drawing rectangle at ({$x1}, {$y1}) size {$w}x{$h}");
        $this->_set_stroke_color($color);
        $this->_set_line_style($width, $cap, "", $style);
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Drawing filled rectangle at ({$x1}, {$y1}) size {$w}x{$h}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting clipping rectangle at ({$x1}, {$y1}) size {$w}x{$h}");
        $this->_pdf->clippingRectangle($x1, $this->y($y1) - $h, $w, $h);
    }

    public function clipping_roundrectangle($x1, $y1, $w, $h, $rTL, $rTR, $rBR, $rBL)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting clipping rounded rectangle at ({$x1}, {$y1}) size {$w}x{$h}");
        $this->_pdf->clippingRectangleRounded($x1, $this->y($y1) - $h, $w, $h, $rTL, $rTR, $rBR, $rBL);
    }

    public function clipping_polygon(array $points): void
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting clipping polygon with " . count($points) . " points");
        // Adjust y values
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Ending clipping");
        $this->_pdf->clippingEnd();
    }

    public function save()
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Saving state");
        $this->_pdf->saveState();
    }

    public function restore()
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Restoring state");
        $this->_pdf->restoreState();
    }

    public function rotate($angle, $x, $y)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Rotating by {$angle} degrees at ({$x}, {$y})");
        $this->_pdf->rotate($angle, $x, $y);
    }

    public function skew($angle_x, $angle_y, $x, $y)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Skewing by ({$angle_x}, {$angle_y}) at ({$x}, {$y})");
        $this->_pdf->skew($angle_x, $angle_y, $x, $y);
    }

    public function scale($s_x, $s_y, $x, $y)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Scaling by ({$s_x}, {$s_y}) at ({$x}, {$y})");
        $this->_pdf->scale($s_x, $s_y, $x, $y);
    }

    public function translate($t_x, $t_y)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Translating by ({$t_x}, {$t_y})");
        $this->_pdf->translate($t_x, $t_y);
    }

    public function transform($a, $b, $c, $d, $e, $f)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Applying transformation matrix");
        $this->_pdf->transform([$a, $b, $c, $d, $e, $f]);
    }

    public function polygon($points, $color, $width = null, $style = [], $fill = false)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Drawing polygon with " . count($points) . " points, fill: " . ($fill ? 'true' : 'false'));
        $this->_set_fill_color($color);
        $this->_set_stroke_color($color);
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Drawing circle at ({$x}, {$y}) radius {$r}, fill: " . ($fill ? 'true' : 'false'));
        $this->_set_fill_color($color);
        $this->_set_stroke_color($color);
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding image: {$img} at ({$x}, {$y}) size {$w}x{$h}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding select field at ({$x}, {$y}) size {$w}x{$h}");
        $pdf = $this->_pdf;
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding textarea at ({$x}, {$y}) size {$w}x{$h}");
        $pdf = $this->_pdf;
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding input field type: {$type} at ({$x}, {$y}) size {$w}x{$h}");
        $pdf = $this->_pdf;
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding text at ({$x}, {$y}): " . substr($text, 0, 50) . (strlen($text) > 50 ? '...' : ''));
        $pdf = $this->_pdf;
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding JavaScript code");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding named destination: {$anchorname}");
        $this->_pdf->addDestination($anchorname, "Fit");
    }

    public function add_link($url, $x, $y, $width, $height)
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Adding link to: {$url} at ({$x}, {$y}) size {$width}x{$height}");
        $y = $this->y($y) - $height;
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Checking font support for character in font: {$font}");
        if ($char === "") {
            return true;
        }
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting text width for font: {$font}, size: {$size}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting font height for font: {$font}, size: {$size}");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting font baseline for font: {$font}, size: {$size}");
        $ratio = $this->_dompdf->getOptions()->getFontHeightRatio();
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting page script");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting page text at ({$x}, {$y})");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Setting page line from ({$x1}, {$y1}) to ({$x2}, {$y2})");
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Creating new page");
        $this->_page_number++;
        $this->_page_count++;
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Streaming PDF: {$filename}");
        if (headers_sent()) {
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Generating PDF output");
        if (!isset($options["compress"])) $options["compress"] = true;
    ...
    {
        SimpleLogger::log('cpdf_logs', __FUNCTION__, "Getting CPDF messages");
